...

  {
    ...
    detail: {
      ...
      uxStrategy: {
        ...
        designRationale:
          '従来の紙・Excelの運用フローを尊重しつつ、デジタル化による利点（自動計算、リアルタイム反映、承認通知）を最大限に活用。既存業務に馴染みやすいUIとしつつ、新しい価値を提供することを重視した。',
      },
      ...
      outcome: {
        results: [
          '総務部門の月次処理時間を60%削減（従来40時間 → 16時間）',
          '承認遅延を45%削減し、精算サイクルを平均7日短縮',
          '従業員アンケートで「使いやすい」と回答した割合が92%',
          'リモートワーク環境下でも円滑に運用でき、全社展開が完了',
        ],
        learnings: [
          '既存業務フローを理解した上で、デジタル化の利点を活かす設計が重要であると実感した',
          'ITリテラシーの異なるユーザーが混在する場合、最も操作に不慣れな層に合わせた設計が必要',
          'システム導入だけでなく、運用マニュアルや研修を含めた展開計画が定着率を左右する',
        ],
      },
    },
  },
